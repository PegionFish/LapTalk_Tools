...
## 构建 / 测试占位（复制到新仓库后补充）
- 构建命令：`npm run dev`、`npm run pack`、`npm run dist`
- 单元 / 集成测试命令：`npm test`
- 端到端 / 验证脚本：`npm run smoke:render`
- 依赖安装需用户手动执行：`corepack pnpm install` 或 `npm install`
...
### 项目特性
- 主要技术栈：`Electron + 原生 HTML/CSS/JavaScript + Node.js + electron-builder`
- 自定义构建/发布注意事项：`当前仓库不自动安装依赖；优先导出透明 MOV / ProRes 4444；FFmpeg 第一阶段通过探测或手动设置路径接入`
- 额外安全/合规约束：`主 UI 窗口保持 contextIsolation；仅隐藏渲染窗口允许关闭 webSecurity 以兼容本地 HTML 互访`
- 仓库特有流程：`重要里程碑完成后必须立即 git commit 并 push 到 origin/main，避免长链路未备份改动`
