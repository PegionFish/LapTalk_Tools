...
- 支持标题、字体下拉选择、线宽、网格、图例位置、纯曲线模式、时间/数值刻度文字显示、时间刻度密度滑块、前后裁剪滑块，以及坐标轴、网格、时间文字、数值文字、图例文字和不同参数的独立颜色设置（颜色可直接输入 16 进制值或使用取色器，如 `66CCFF`）
...
1. 点击“浏览...”选择 HWiNFO CSV 文件，或将 CSV 文件直接拖入窗口
2. 在左侧筛选并多选要绘制的参数
3. 按需设置标题、字体、线宽、时间刻度密度、固定时间间隔、前后裁剪范围、图例、时间/数值刻度文字显示、输出宽高和 DPI，并可为坐标轴、网格、时间文字、数值文字、图例文字和不同参数指定颜色；颜色支持直接输入 `66CCFF` / `#66CCFF` 或使用取色器
